...
def clean_data(data):
    ...
    #print(cleaned_data)
    cleaned_data[['cycles']] = cleaned_data[['cycles']].round(4)
    ...
    cleaned_data[["dram energy"]] = cleaned_data[["dram energy"]].round(4)
...
def plot_graph(X, y1, y2):
    ...
    ax1.set_ylabel('DRAM Energy (Test)', color=color)
    ...
    color = 'tab:blue'
    ax2.set_ylabel('DRAM Energy (Predicted)', color=color)
    ax2.scatter(X, y2, color=color)
    ax2.tick_params(axis='y', labelcolor=color)

    # Title and grid
    plt.title('Graph of Cycles vs DRAM Energy')
    ax1.grid(True)
...
def plot_2d_graph(X,Y):
    ...
    # Adding title and labels
    plt.title('DRAM Energy (Test vs Predicted)')
    plt.xlabel('DRAM Energy (Test)')
    plt.ylabel('DRAM Energy (Predicted)')
...
history = model.fit(X_train_scaled, y_train_scaled, validation_split=0.2, epochs=400, batch_size=64, verbose=1)
