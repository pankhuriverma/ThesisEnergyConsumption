...
if __name__ == "__main__":
    ...
    total_time = []
    all_events = {}
    events = [papi_events.PAPI_TOT_INS, papi_events.PAPI_TOT_CYC]
    ...
    event_names = ["Total Instructions", "Total Cycles"]
    ...
    for event, event_name in zip(events, event_names):
        ...
        for i in range(X.shape[1]):  # X.shape[1] gives the number of columns (features)
                # Select the current feature
                X_feature = X[:, i:i+1]   # Selecting a single feature column; reshaping is not required here

                # Split the data into training/testing sets
                X_train, X_test, y_train, y_test = train_test_split(X_feature, y, test_size=0.2, random_state=42)

                perf_result, cpu_ene, dram_ene, total_ene, tot_time = measure_model_perf_energy(event, X_train, y_train)
    ...
    csv_file = '../../dataset/old_datasets/ML_model_collected_dataset_ins_cyc_new.csv'  # Specify your CSV file name
